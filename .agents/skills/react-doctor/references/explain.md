...
```bash
nub dlx react-doctor@latest rules explain react-doctor/no-array-index-as-key
```
...
```bash
nub dlx react-doctor@latest --verbose --diff
```

## Commands

```bash
nub dlx react-doctor@latest rules list                         # every rule + its effective severity
nub dlx react-doctor@latest rules list --configured            # only what your config changed
nub dlx react-doctor@latest rules list --category Performance   # filter by category
nub dlx react-doctor@latest rules explain <rule>               # why it matters + how to configure
nub dlx react-doctor@latest rules disable <rule>               # rule never runs
nub dlx react-doctor@latest rules enable <rule>                # turn back on at its recommended severity
nub dlx react-doctor@latest rules set <rule> warn              # off | warn | error
nub dlx react-doctor@latest rules category "React Native" off   # whole category
nub dlx react-doctor@latest rules ignore-tag design            # skip a rule family (design, test-noise, …)
nub dlx react-doctor@latest rules unignore-tag design
```
